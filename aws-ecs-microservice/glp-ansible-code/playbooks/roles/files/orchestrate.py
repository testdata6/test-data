...
parser=argparse.ArgumentParser(description='Arguments for managing PLP-services with ecs')
...
action.add_argument('--run',action='store_true',help='REQUIRED/EXCLUSIVE : Create a task def without creating service')
...
def runTask(mode, confFile, region, count, clusterName ):
    ...
    for modules in confParser['containers']:
        for task in modules['runTask']:
            if count == int(-1): 
                taskDesiredCount = task['desiredCount']
            else: 
                taskDesiredCount = count
            taskCluster = clusterName
            taskDefinition = task["taskDefinition"]
            
            try:
                if mode:
                    print "Running Task"
                tskRunResponse = client.run_task(cluster = taskCluster, taskDefinition = taskDefinition, count = taskDesiredCount)
                RunResponse = tskRunResponse['ResponseMetadata']['HTTPStatusCode']
                if RunResponse == 200:
                    if mode:
                        print RunResponse
                        print "Response::: %s" %(RunResponse)
                    return RunResponse
                else: return failedResponseCode
            except Exception, e:
                if mode:
...
def main(mode,confFile,region,count,action):
    ...
    for modules in confParser['containers']:
            ...
            if isinstance(svc['cluster'],list):
               ...
               if action == 'create':
                  for cluster in svc['cluster']:
                      print cluster
                      response = createService(mode,confFile,region,count,cluster)
        ...
        #     if isinstance(task['cluster'],list):
        #         if mode:
        #             print "Multiple Custers for task"
        #         if action == 'run':
        #             for cluster in task['cluster']:
        #                 print cluster
        #                 response = runTask(mode, confFile, region, count, cluster)
        #                 responseCode = response
        #     else:
        #         if mode:
        #             print "Single cluster for single task"
        #         if action == 'run':
        #             response = runTask(mode, confFile, region, count, taskCluster)
        #             responseCode = response
...
if __name__ == '__main__':
   ...
   if args.run:
     action = 'run'
     print "Run Task Request Recieved.......\n"
     if confFile:
        taskConfFile = confFile
        if mode:
          print "Task config file : %s " %(taskConfFile)
        main(mode, taskConfFile, region, count, action)
     else:
        for confFile in os.listdir(os.path.dirname(os.path.abspath(__file__))+"/config/" + args.env + "/"):
            if confFile.endswith(".yaml"):
                serviceConfFile = os.path.dirname(os.path.abspath(__file__))+"/config/"+args.env+"/"+confFile
