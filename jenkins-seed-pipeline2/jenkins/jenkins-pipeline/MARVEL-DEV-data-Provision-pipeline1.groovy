...
//    def s3FolderName = indextype.reverse()

    try{
    result = build job: "Marvel-Dev-main-data-provision-k8s",
        parameters:
...
//        echo "${buildVersion} Incorrect version !! Seems copy paste issue"
    ...
    sh'kubectl config use-context infra-clusters.kube-nonprod.plp-gmail.com'
...
node(){
    ...
    try{
    healthResult = build job: 'Marvel-DEV-main-deployer-k8s',
        parameters:
           [[$class: 'StringParameterValue', name: 'NAMESPACE', value: "${NAMESPACE}"],
            [$class: 'StringParameterValue', name: 'SERVICE_NAME', value: "${SERVICE_NAME}"],
            [$class: 'StringParameterValue', name: 'CB_CLUSTER', value: "${CB_CLUSTER}"],
...
node(){
    ...
    print "KUBE CLEANUP NOT REQUIRE"
