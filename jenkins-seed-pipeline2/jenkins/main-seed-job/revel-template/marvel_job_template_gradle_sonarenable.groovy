...
job("PLP-Marvel-Gradle-Builder-SERVICENAME") {
    ...
    multiscm {
      ...
      remote {
       ...
       extensions {
          ...
          remote {
	    credentials 'AAAAAAAAAAAAA'
            url "ssh://git@github.com/plpv2/plp-core.git"
                 }
           branch "stage-master"
           extensions {
                     wipeOutWorkspace()
                     relativeTargetDirectory('core')
                      }
  ...
  /*     shell ('wget -O - -o /dev/null  --user ${nexus_user} --password ${nexus_pass} https://my-nexus.com/nexus-deps/content/repositories/plp-releases/com/mongo/platform/common/maven-metadata.xml | grep -Po \'(?<=<version>)([0-9\\.]+(-SNAPSHOT)?)\' | sort --version-sort -r | head -n 1 > rel.txt \
        ...
        shell('rm -rf build.gradle \
		        \ncp core/build-files/build.gradle . \
				\ncp core/build-files/qualityplugin.gradle . \
                         \nrm -rf core')

        gradle {
                tasks ('clean build publish -Pvalidate=true --no-daemon --max-workers 2')
                useWrapper(true)
                makeExecutable(true)
                }

        shell ('gitIDTag=`echo ${GIT_COMMIT} | cut -c 1-10` \
              \n echo ${gitIDTag}_RVbNo${BUILD_NUMBER} > version.txt \
              \ncp build/libs/*exec.jar . ')
...
*/
    ...
    it / 'builders' << 'hudson.plugins.sonar.SonarRunnerBuilder' {     // SONARQUBE_CONFIG
          ...
          properties 'sonar.projectKey=PLP-MARVELl-SERVICENAME\nsonar.projectName=PLP-MARVELl-SONARPROJECTNAME\nsonar.java.binaries=build/classes/java' // SONARQUBE_CONFIG
